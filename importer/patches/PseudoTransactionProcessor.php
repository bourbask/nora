...
class PseudoTransactionProcessor
{
    ...
    private function getDefaultCurrency(): void
    {
        ...
        // Firefly III v6.1+ removed GET /api/v1/preferences/currencyPreference,
        // replaced by GET /api/v1/currencies/default.
        try {
            $response     = Http::withToken($token)
                ->timeout((int) config('importer.connection.timeout'))
                ->get(sprintf('%s/api/v1/currencies/default', $url));
            if ($response->successful()) {
                $data     = $response->json();
                $code     = $data['data']['attributes']['code'] ?? 'EUR';
            }
        } catch (\Exception $e) {
            app('log')->error($e->getMessage());
        }
